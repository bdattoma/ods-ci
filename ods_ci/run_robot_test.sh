...
### TEMP solution for urnning ODS-CI as container in shiftleft pipeline
...
if ${EMAIL_REPORT}
  then
     ...
     poetry run python3 utils/scripts/Sender/send_report.py send_email_report -s "${EMAIL_FROM}" -r "${EMAIL_TO}" -b "ODS-CI: Run Results" \
